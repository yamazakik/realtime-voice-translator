...
const splitIntoSentences = (text: string): string[] => {
  ...
  return sentences ? sentences.map(s => s.trim()).filter(s => s.length > 0) : [text.trim()].filter(t => t.length > 0);
};
...
const App: React.FC = () => {
  ...
  const [displayedJapaneseSegments, setDisplayedJapaneseSegments] = useState<string[]>([]);
  const [fullEnglishTranslation, setFullEnglishTranslation] = useState<string>('');
  const [displayedEnglishSentences, setDisplayedEnglishSentences] = useState<string[]>([]);

  const [textAreaHeight, setTextAreaHeight] = useState<number>(256);
  const [fontSize, setFontSize] = useState<number>(16);

  const increaseHeight = () => setTextAreaHeight(h => Math.min(MAX_TEXT_AREA_HEIGHT, h + HEIGHT_STEP));
  const decreaseHeight = () => setTextAreaHeight(h => Math.max(MIN_TEXT_AREA_HEIGHT, h - HEIGHT_STEP));
  const increaseFontSize = () => setFontSize(s => Math.min(MAX_FONT_SIZE, s + FONT_STEP));
  const decreaseFontSize = () => setFontSize(s => Math.max(MIN_FONT_SIZE, s - FONT_STEP));

  const executeTranslation = useCallback(async (textToTranslate: string) => {
    if (!textToTranslate.trim()) {
      if (lastSubmittedTextForTranslationRef.current === textToTranslate || !lastSubmittedTextForTranslationRef.current) {
    ...
    try {
      const translation = await translateText(textToTranslate, "English", "日本語");
  ...
  const triggerTranslationDebounced = useCallback((textToTranslate: string) => {
    ...
    if (!textToTranslate.trim()) {
      ...
      if (!currentSessionFinalTranscriptRef.current.trim() && !interimTranscript.trim()) {
         setFullEnglishTranslation('');
         setIsLoadingTranslation(false);
      }
      return; 
  ...
  const handleStartListening = useCallback(() => {
    if (!('SpeechRecognition' in window || 'webkitSpeechRecognition' in window)) {
      setError('お使いのブラウザは音声認識をサポートしていません。Chromeの使用をお勧めします。');
      return;
    }
    ...
    recognition.onend = () => {
      ...
      const finalTranscriptForSession = currentSessionFinalTranscriptRef.current.trim();
      if (finalTranscriptForSession && finalTranscriptForSession !== lastSubmittedTextForTranslationRef.current) {
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      if (event.error === 'network') errorMessage = 'ネットワークエラー。接続を確認してください。';
      else if (event.error === 'not-allowed' || event.error === 'service-not-allowed') errorMessage = 'マイクアクセスが許可されていません。';
  ...
  const japaneseTextForDisplay = displayedJapaneseSegments.join(' ') +
                                 (interimTranscript ? (displayedJapaneseSegments.length > 0 ? ' ' : '') + interimTranscript : '');
  const englishTextForDisplay = displayedEnglishSentences.join(' ');
  const currentDynamicStyle = { height: `${textAreaHeight}px`, fontSize: `${fontSize}px` };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 text-slate-100 flex flex-col items-center p-4 selection:bg-indigo-500 selection:text-white">
      <header className="w-full max-w-4xl text-center my-8">
        ...
        <p className="text-slate-400 mt-2">日本語の音声を文字起こしし、英語に翻訳します。（直近 約{MAX_TEXT_ITEMS_FOR_DISPLAY}文表示）</p>
      </header>
      ...
      <main className="w-full max-w-4xl flex flex-col items-center space-y-6">
        ...
        {error && (
          <div className="w-full bg-red-700/50 border border-red-500 text-red-200 px-4 py-3 rounded-lg shadow-md" role="alert">
        ...
        <div className="w-full max-w-2xl flex flex-col sm:flex-row justify-around items-center space-y-4 sm:space-y-0 sm:space-x-2 my-2 p-4 bg-slate-800/60 rounded-lg shadow-lg">
          <div className="flex items-center space-x-2">
            <span className="text-slate-300 font-medium text-sm">表示枠の高さ:</span>
            <Button onClick={decreaseHeight} className="p-1.5" aria-label="表示枠を縮小" disabled={textAreaHeight <= MIN_TEXT_AREA_HEIGHT} variant="secondary">
              <MinusIcon />
            </Button>
            <span className="text-slate-200 w-12 text-center text-sm tabular-nums">{Math.round(textAreaHeight / 16)} rem</span>
            <Button onClick={increaseHeight} className="p-1.5" aria-label="表示枠を拡大" disabled={textAreaHeight >= MAX_TEXT_AREA_HEIGHT} variant="secondary">
              <PlusIcon />
            </Button>
          </div>
          <div className="flex items-center space-x-2">
            <span className="text-slate-300 font-medium text-sm">文字の大きさ:</span>
            <Button onClick={decreaseFontSize} className="p-1.5" aria-label="文字を縮小" disabled={fontSize <= MIN_FONT_SIZE} variant="secondary">
               <MinusIcon />
            </Button>
             <span className="text-slate-200 w-12 text-center text-sm tabular-nums">{fontSize}px</span>
            <Button onClick={increaseFontSize} className="p-1.5" aria-label="文字を拡大" disabled={fontSize >= MAX_FONT_SIZE} variant="secondary">
              <PlusIcon />
            </Button>
          </div>
        </div>
      ...
      <footer className="w-full max-w-4xl text-center text-slate-500 mt-12 pb-8">
        <p>&copy; {new Date().getFullYear()} 音声翻訳アプリ. Powered by Gemini API & Web Speech API.</p>
      </footer>
    </div>
  );
};
